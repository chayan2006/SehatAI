...
export default function DoctorBilling() {
  ...
  const repairDatabase = () => {
    const sql = `-- FIX: Create missing columns in billing_records
...
BEGIN
    ...
    DROP POLICY IF EXISTS "Global Demo Access" ON public.billing_records;
    CREATE POLICY "Global Demo Access" ON public.billing_records FOR ALL USING (true) WITH CHECK (true);
END $$;`;
    navigator.clipboard.writeText(sql);
    addToast('Repair SQL copied to clipboard!', 'success', 3000);
  };
  ...
  const handleCreate = async (invoiceData) => {
    ...
    const lid = addToast(`Creating invoice...`, 'loading', 3000);
    try {
      const hospital = await hospitalService.getMyHospital();
  ...
  const handleUpdateStatus = async (id, status, patientName) => {
    setProcessingId(id);
    const lid = addToast(`Updating invoice status...`, 'loading', 3000);
    try {
  ...
  const handleDownloadPDF = async (inv) => {
    const lid = addToast(`Generating PDF for ${inv.id.slice(0,8)}...`, 'loading', 3000);
  ...
  const handleFinalizeAndSend = async (inv) => {
    ...
    const lid = addToast(`Finalizing invoice & sending to ${inv.patients?.full_name}...`, 'loading', 3000);
    try {
  ...
  return (
    <div className="space-y-6 p-2 animate-in fade-in duration-500">
        ...
        <div className="flex gap-3">
          {lastError && (
             <button 
               ...
               className="flex items-center gap-2 px-5 h-11 bg-red-50 text-red-600 rounded-xl font-bold text-xs uppercase tracking-widest border border-red-100 hover:bg-red-100 transition-all shadow-sm"
             >
               <RotateCcw size={16}/> Auto-Fix DB
             </button>
          )}
          <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Revenue',      val: `₹${totalRevenue.toLocaleString()}`, icon: IndianRupee, color: 'emerald', sub: 'from paid invoices' },
              ...
              return (
                <div key={i} className="flex-1 group relative flex flex-col items-center">
                  <div className="absolute -top-7 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[9px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition whitespace-nowrap pointer-events-none z-10">
                    ₹{bar.day === 'Mon' ? '6800' : bar.amount.toLocaleString()}
                  </div>
                  <div

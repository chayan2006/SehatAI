    ...
    <div className="p-8 space-y-8 animate-in fade-in duration-500">
      ...
      <section className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900/50 p-6 rounded-2xl shadow-[0_10px_30px_-5px_rgba(0,0,0,0.03)] border border-slate-100 dark:border-slate-800 group hover:translate-y-[-2px] transition-all">
          ...
          <h3 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">156</h3>
        </div>
        ...
        <div className="bg-red-50/50 dark:bg-red-900/10 p-6 border border-red-100 dark:border-red-900/20 rounded-2xl shadow-[0_10px_30px_-5px_rgba(239,68,68,0.05)] border-l-4 border-l-red-500 group hover:translate-y-[-2px] transition-all">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-[10px] font-bold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-2 py-1 rounded-full tracking-wider">CRITICAL</span>
          </div>
          <p className="text-slate-500 dark:text-slate-400 text-[10px] font-black tracking-widest uppercase mb-1">ER Wait Time</p>
          <h3 className="text-3xl font-extrabold tracking-tight text-red-600 dark:text-red-400">24<span className="text-lg font-normal ml-1">min</span></h3>
        </div>
      </section>
              ...
              {[40, 60, 45, 80, 95, 55, 30, 45].map((height, i) => (
                <div key={i} className="flex-1 bg-slate-50 dark:bg-slate-800/50 rounded-t-xl h-full relative group">
                  <div 
                    ...
                    style={{ height: `${height}%` }}
                  ></div>
                  {/* Internal Bar for alert density */}
                  <div 
            ...
            <div className="space-y-8">
              <div>
                <div className="flex justify-between items-center mb-3">
                  ...
                  <span className="text-[10px] font-black text-primary px-2 py-1 bg-primary/10 rounded-full tracking-tighter">12 / 14</span>
                </div>
              ...
              <div className="pt-6">
                <div className="space-y-4">
                  {[
                    { icon: 'check_circle', text: 'Lab-04 throughput optimized to ', val: '15m/test', color: 'primary' },
                    { icon: 'sync', text: 'Blood Bank restock: ', val: 'O-Negative critical', color: 'orange' },
                    { icon: 'verified', text: 'Generator System Test ', val: 'PASSED', color: 'primary' }
                  ].map((log, i) => (
                    <div key={i} className="flex gap-4 group">
                      <span className={`material-symbols-outlined text-lg text-${log.color === 'primary' ? 'primary' : 'orange-500'} group-hover:scale-110 transition-transform`}>{log.icon}</span>
                      <p className="text-[11px] font-bold leading-relaxed text-slate-500 dark:text-slate-400">
                        {log.text}
                        <span className="text-slate-900 dark:text-white font-black">{log.val}</span>
                      </p>
                    </div>
                  ))}
                </div>
      ...
      <button className="fixed bottom-10 right-10 w-16 h-16 bg-primary text-white rounded-2xl shadow-2xl shadow-primary/40 flex items-center justify-center group active:scale-95 transition-all z-40 hover:rotate-3">
        <span className="material-symbols-outlined text-3xl font-black">add</span>
        <span className="absolute right-full mr-6 bg-slate-900 text-white px-4 py-2 rounded-xl text-xs font-black tracking-widest opacity-0 group-hover:opacity-100 whitespace-nowrap pointer-events-none transition-all shadow-xl -translate-x-2 group-hover:translate-x-0">
          NEW ADMISSION
        </span>
      </button>
    </div>
